export const movies = [
  {
    id: 1,
    name: "Moonligth",
    image: "../images/pelicula3.jpg",
    description:
      "Chiron, un joven afroamericano que vive en una zona conflictiva de Miami, tiene problemas para aceptar su homosexualidad. Mientras madura en un ambiente hostil, experimenta la alegría, la ira, el placer de la belleza, el éxtasis y el dolor.",
  },
  {
    id: 2,
    name: "Titanic",
    image: "https://m.media-amazon.com/images/I/51tP7EBwsvL._AC_.jpg",
    description:
      "Jack es un joven artista que gana un pasaje para viajar a América en el Titanic, el transatlántico más grande y seguro jamás construido.",
  },
  {
    id: 3,
    name: "The Prodigy",
    image: "../images/pelicula2.jpg",
    description:
      "La vida de Sarah se convierte en un infierno cuando Miles, su hijo de 8 años, extremadamente inteligente, empieza a comportarse de manera perturbadora y violenta. Después de consultar a dos expertos, Sarah sospecha que Miles está poseído.",
  },
  {
    id: 4,
    name: "Avatar",
    image: "../images/pelicula4.webp",
    description:
      "Avatar (comercializada como Avatar de James Cameron) es una película épica de ciencia ficción estadounidense de 2009, escrita, producida y dirigida por James Cameron y protagonizada por Sam Worthington, Zoe Saldaña, Sigourney Weaver, Stephen Lang y Michelle Rodriguez.",
  },
  {
    id: 5,
    name: "Black Swan",
    image: "../images/pelicula5.jpg",
    description:
      "Una brillante bailarina que forma parte de una compañía de ballet de Nueva York, vive completamente absorbida por la danza. La presión de sus controladora madre y la rivalidad con su compañera Lily afectarán terriblemente a la niña.",
  },
  {
    id: 6,
    name: "Hollywood",
    image: "../images/pelicula6.jpg",
    description:
      "A finales de los 60, Hollywood empieza a cambiar y el actor Rick Dalton trata de adaptarse a los nuevos tiempos. Junto a su doble, ambos experimentan problemas para modificar sus hábitos, debido a lo enraizados que están. Al mismo tiempo, nace una relación entre Rick y la actriz Sharon Tate, que fue víctima de la familia Manson en la matanza de 1969.",
  },
  {
    id: 7,
    name: "Unidos",
    image: "../images/pelicula8.jpg",
    description:
      "El doctor Hank Pym anuncia que abandona S.H.I.E.L.D. tras descubrir que han tratado de apropiarse de la tecnología de su Ant-Man, que Pym considera altamente peligrosa. Varios años después, Scott Lang sale de prisión tras cumplir tres años de condena por robo.",
  },
  {
    id: 8,
    name: "Iron Man",
    image: "../images/pelicula9.jpg",
    description:
      "Tony Stark es un inventor de armamento brillante que es secuestrado en el extranjero. Sus captores son unos terroristas que le obligan a construir una máquina destructiva pero Tony se construirá una armadura para poder enfrentarse a ellos y escapar.",
  },
  {
    id: 9,
    name: "Thor",
    image: "../images/pelicula11.jpg",
    description:
      "Tras desatar una antigua guerra, el codicioso guerrero Thor es desterrado a la Tierra por su padre para que viva entre los hombres y descubra así el verdadero sentido de la humildad. Allí, sin sus poderes, Thor deberá enfrentarse a las fuerzas más oscuras que su mayor enemigo le enviará desde Asgard.",
  },
  {
    id: 10,
    name: "Avengers",
    image: "../images/pelicula12.jpg",
    description:
      "El director de la Agencia SHIELD decide reclutar a un equipo para salvar al mundo de un desastre casi seguro cuando un enemigo inesperado surge como una gran amenaza para la seguridad mundial.",
  },
  {
    id: 11,
    name: "Ant-Man",
    image: "../images/pelicula13.jpg",
    description:
      "El doctor Hank Pym anuncia que abandona S.H.I.E.L.D. tras descubrir que han tratado de apropiarse de la tecnología de su Ant-Man, que Pym considera altamente peligrosa. Varios años después, Scott Lang sale de prisión tras cumplir tres años de condena por robo.",
  },
  {
    id: 12,
    name: "Black Widow",
    image: "../images/pelicula14.jpg",
    description:
      "Una peligrosa conspiración, relacionada con su pasado, persigue a Natasha Romanoff, también conocida como Viuda Negra. La agente tendrá que lidiar con las consecuencias de haber sido espía, así como con las relaciones rotas, para sobrevivir.",
  },
  {
    id: 13,
    name: "Shang-chi",
    image: "../images/pelicula16.jpg",
    description:
      "El maestro de artes marciales Shang-Chi se enfrenta al pasado que creía haber dejado atrás cuando se ve envuelto en la red de la misteriosa organización de los Diez Anillos.",
  },
  {
    id: 14,
    name: "Increibles 2",
    image: "../images/pelicula15.jpeg",
    description:
      "Helen es reclutada para ayudar a que la acción vuelva a la vida de los Súper, mientras Bob se enfrenta a la rutina de su vida diaria normal en el hogar. En casa debe lidiar con un bebé que está a punto de descubrir sus superpoderes.",
  },
];
